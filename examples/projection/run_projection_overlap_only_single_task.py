...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run an overlap-only projection task for a single subdirectory. "
        "Outputs overlap.h5 and a dummy (zero-filled) hamiltonian.h5 to enable sequential projections."
    )
    parser.add_argument(
        "--path",
        type=Path,
    ...
    parser.add_argument(
        ...
        default=[9, 9, 1],
        metavar=("NX", "NY", "NZ"),
        help="k-grid dimensions (default: 9 9 1)",
    )
...
def main() -> None:
    ...
    kgrid = tuple(cast(tuple[int, int, int], tuple(args.kgrid)))
    ...
    print(f"Output contains: overlap.h5 (projected), hamiltonian.h5 (dummy zeros), info.json, POSCAR")
